...
import { extname, join, normalize } from "$std/path/mod.ts";
...
function toTemplate(root: string, entries: {
  ...
  for (const i in entries) {
    ...
    const file = "./" + normalize(join(rootPath, path));
    const extension = extname(file);
    const rawRoute = file.slice(rootPath.length, -extension.length);
    const route = rawRoute.endsWith("/index")
      ? rawRoute.slice(0, -"index".length)
      : rawRoute;

    const isScript = scriptExtensions.some((ext) => file.endsWith(ext));
    const varName = (isScript ? "$" : "") + i;
...
const stringifyTemplate = ({ imports, assets, routes }: Template) => `
